...
function appendAccessLog(method, route) {
  ...
  fs.appendFile(accessLogFile, line, "utf8", (error) => {
    if (error) console.error("No fue posible registrar el acceso:", error.message);
  });
}
...
function appendTransactionError(message) {
  ...
  fs.appendFile(transactionLogFile, line, "utf8", (error) => {
    if (error) console.error("No fue posible registrar la transacción:", error.message);
  });
}

module.exports = { appendAccessLog, appendTransactionError };
